...
async function fetchSubtitles(imdbId, type, season, episode) {
  try {
    let subtitlesUrl = `${API_BASE_URL}/subtitles/search?imdbId=${imdbId}&languages=pt,en,es,fr`;
    ...
    if (!response.ok) {
      console.warn("Erro ao buscar legendas:", response.status, response.statusText);
...
async function getSubtitleDownloadUrl(fileId) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(`Erro ao obter URL de download do backend: ${response.status}. Detalhes: ${JSON.stringify(errorData)}`);
    }
    ...
    console.error("Erro ao obter URL de download da legenda via backend:", error);
    return null;
  }
}
...
function createSubtitlePicker(subtitles, onSubtitleSelect) {
  ...
  badge.textContent = `${subtitles.length} encontrada${subtitles.length !== 1 ? 's' : ''}`;
  badge.style.cssText = `
    background: #0066cc;
    color: #fff;
    padding: 4px 8px;
  ...
  Object.entries(groupedSubtitles).forEach(([language, langSubtitles]) => {
    ...
    const sortedSubtitles = langSubtitles.sort((a, b) => {
      if (a.from_trusted !== b.from_trusted) return b.from_trusted - a.from_trusted;
    ...
    sortedSubtitles.slice(0, 3).forEach((subtitle, index) => {
      ...
      const badges = [];
      if (subtitle.hd) badges.push('<span style="background: #00cc00; color: #fff; padding: 2px 4px; border-radius: 3px; font-size: 0.7em; margin-left: 4px;">HD</span>');
      if (subtitle.from_trusted) badges.push('<span style="background: #0066cc; color: #fff; padding: 2px 4px; border-radius: 3px; font-size: 0.7em; margin-left: 4px;">✓</span>');
      if (subtitle.hearing_impaired) badges.push('<span style="background: #cc6600; color: #fff; padding: 2px 4px; border-radius: 3px; font-size: 0.7em; margin-left: 4px;">CC</span>');
      if (subtitle.ai_translated) badges.push('<span style="background: #9966cc; color: #fff; padding: 2px 4px; border-radius: 3px; font-size: 0.7em; margin-left: 4px;">AI</span>');

      const releaseText = subtitle.release || 'Release desconhecido';
      const truncatedRelease = releaseText.length > 35 ? releaseText.substring(0, 35) + '...' : releaseText;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (imdbId && type) {
    try {
      ...
      let streams = (torrentioData.streams || []).filter((s) => s.infoHash);

      if (!streams.length) throw new Error("Nenhum stream encontrado no Torrentio.");
      ...
      streams = streams.filter((stream) => {
        ...
        return !(t.includes('.rar') || t.includes('.zip') || t.includes('.7z') || t.includes('.txt') || t.includes('.nfo') || t.includes('sample'));
      });

      if (!streams.length) throw new Error("Nenhum stream de vídeo disponível após a filtragem inicial. Por favor, tente outro título.");
      ...
      const availableSubtitles = await fetchSubtitles(imdbId, type, season, episode);
      ...
      if (availableSubtitles.length > 0) {
        const subtitlePicker = createSubtitlePicker(availableSubtitles, async (subtitle, language) => {
          console.log(`Legenda selecionada: ${subtitle.release} (${language})`);
          ...
          if (picker) {
            ...
            successMsg.innerHTML = `
              ...
              <strong>Legenda selecionada:</strong> ${getLanguageName(language)} - ${subtitle.release || 'Legenda'}
            `;
        ...
        btn.onclick = async () => {
          ...
          try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Erro ao buscar o stream: ${response.status} ${response.statusText}. Resposta: ${errorText}`);
            }
            const streamData = await response.json();
            if (!streamData.success || !streamData.url) {
                throw new Error(streamData.message || "Erro ao obter o URL do stream. Sucesso: " + streamData.success + " URL: " + streamData.url);
            }
            ...
            videoElement.addEventListener("loadeddata", () => {
                loading.remove();
                console.log("Vídeo carregado com sucesso. Metadados disponíveis.");
            });
            videoElement.addEventListener("error", (e) => {
                loading.remove();
                console.error("Erro no elemento <video> nativo:", e);
                let videoErrorMessage = "O vídeo não pôde ser carregado. Tente outra fonte ou verifique sua conexão.";
                if (videoElement.error) {
                    switch (videoElement.error.code) {
                        ...
                        case videoElement.error.MEDIA_ERR_DECODE:
                            videoErrorMessage += " (Erro de decodificação de vídeo/áudio).";
                            break;
                        case videoElement.error.MEDIA_ERR_SRC_NOT_SUPPORTED:
                            videoErrorMessage += " (Formato de mídia não suportado ou URL inválido).";
                            break;
                        default:
                            videoErrorMessage += " (Erro desconhecido: " + videoElement.error.code + ").";
                            break;
                    }
                }
                showError(videoErrorMessage + " URL do stream: " + streamData.url);
            });
      ...
      console.error("Erro geral ao preparar o stream (antes da seleção da fonte):", error);
      showError(error.message || "Erro ao preparar o vídeo. Por favor, recarregue a página e tente novamente.");
    }
  } else {
